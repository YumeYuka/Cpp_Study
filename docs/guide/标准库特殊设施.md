...
:::info string的下标编号习惯于bitset恰好相反：string中下标最大的字符（最右）用来初始化bitset中的低位。
:::
...
:::info C++程序不应该使用库函数rand，而应使用default_random_engine类和恰当的分布类对象。
:::
...
:::info 当我们说随机数发生器时，是指分布对象和引擎对象的组合。
:::
